...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//MANUALL SETUP OF NO OF CREEPS IN MEMORY OF SPAWN
...
*/
        ...
        if (name == undefined|| name < 0){
            ...
            //spawn transporters
            //Upgraders have exclusive energy source by setting source number for upgrader to 1.
            ...
            if((numberOfTransporters < spawn.memory.minTransporters) && containers){
                ...
                name = spawn.createCustomCreep(energy, 'upgrader', 1, HOME);
            }
            else if (numberOfRepairers < spawn.memory.minRepairers){
                name = spawn.createCustomCreep(energy, 'repairer', 0, HOME);
            }
            else if (numberOfBuilders < spawn.memory.minBuilders){
                name = spawn.createCustomCreep(energy, 'builder', 0, HOME);
        ...
        if (!(name < 0) && name != undefined){
            ...
            console.log(spawnName + ' spawning ' + Game.creeps[name].memory.role + ' ' + name);
        }
        ...
        for (let name in Game.creeps){
            ...
            if (creep.ticksToLive <= 50 && creep.memory.role != 'miner'&& creep.hitsMax <= 300){
                creep.moveTo(spawn);
            }
            
            //auto renew creep when the creep is near spawn
            if (creep.pos.isNearTo(spawn) && creep.ticksToLive < 750){
